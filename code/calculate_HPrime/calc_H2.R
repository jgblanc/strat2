...
for (i in 1:22) {
  ...
  for (b in 44:44) {
    ...
    print(nrow(dfR_block))
    print(head(dfR_block))
    print(tail(dfR_block))
    r_ids <- dfR_block$ID
    print(length(r_ids))
    ...
    x <- dfR_blocks %>%	filter(r_ids %in% dfR_blocks$ID)
    print(paste0("The dim of matBlock is ", dim(matBlock)))
    print(paste0("The dim of matBlock is ", dim(as.matrix(x))))
    matf <- matBlock %*% x
