...
dfr <- cbind(dfPVar, r)
dfr <- as.data.frame(dfr) 
fwrite(dfr[1:500,], "~/Downloads/dfr1.txt", quote = F, row.names = F, sep = "\t")
fwrite(dfr[501:1000,], "~/Downloads/dfr2.txt", quote = F, row.names = F, sep = "\t")
fwrite(dfr[1001:1500,], "~/Downloads/dfr3.txt", quote = F, row.names = F, sep = "\t")
...
plinkFGr1 <- (1/sqrt(499)) * plinkFGr1_raw$V1_SUM
var(plinkFGr1)
...
plinkFGr2 <- (1/sqrt(499)) * plinkFGr2_raw$V1_SUM
var(plinkFGr2)

# Run plink command 
plink_cmd <- paste0("~/Downloads/plink2 --pfile ~/Downloads/ukb_imp_chr21_v3 --score ~/Downloads/dfr3.txt variance-standardize header-read cols=dosagesum,scoresums  --out ~/Downloads/FGr_plink3")
system(plink_cmd)

# Read in results
plinkFGr3_raw <- fread("~/Downloads/FGr_plink3.sscore")
plinkFGr3 <- (1/sqrt(499)) * plinkFGr3_raw$V1_SUM
var(plinkFGr3)


# Combine 
plinkFGrT1 <- (plinkFGr1+ plinkFGr2 + plinkFGr3) * (1/sqrt(3)) 
var(plinkFGrT1)
plot(sort(plinkFGrT1),sort(FGr))

# Combine 
plinkFGrT2 <- (plinkFGr1_raw$V1_SUM + plinkFGr2_raw$V1_SUM + plinkFGr3_raw$V1_SUM) * (1/sqrt(1500-1))
var(plinkFGrT2)
plot(sort(plinkFGrT2),sort(FGr))

plot(sort(plinkFGrT1),sort(plinkFGrT1))
```
