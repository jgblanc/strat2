...
## Plot R2 + Ratio Even/Odd Common+Rare PCA
...
# Restructure PC names 

# Custom x-axis breaks and labels
breaks <- 1:80
labels <- c(1:40, paste0("<span style='color:red;'>", 1:40, "</span>"))
```
...
# Your plot with the updated scale_x_continuous
dfPlot %>% 
  ggplot(aes(x = PC, y = R2, color = gwas)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~contrasts, nrow = 1) + 
  geom_hline(aes(yintercept = Signal, color = gwas), linetype = "dashed") + 
  theme_bw(base_size = 10) + 
  scale_color_manual(values = c("purple1", "darkorange", "skyblue2", "seagreen"), 
                     name= TeX("Sampling distance ($\\epsilon$)")) + 
  scale_x_continuous(breaks = breaks, labels = labels) + 
  theme(
    legend.position = "none", 
    strip.background = element_rect(colour = "black", fill = "white"),
    axis.text.x = element_markdown(size = 10)  # This allows colored text
  ) + 
  xlab("PC") + 
...
plLat <- dfPlot %>% ggplot(aes(x = PC, y = R2, color = gwas)) + geom_point() + geom_line() + facet_wrap(~contrasts, nrow = 1) + geom_hline(aes(yintercept = Signal, color = gwas), linetype = "dashed") + theme_bw(base_size = 10) + scale_color_manual(values = c("purple1", "darkorange", "skyblue2", "seagreen"), name= TeX("Sampling distance ($\\epsilon$)")) + theme( legend.position = "none", strip.background=element_rect(colour="black",fill="white"), axis.text.x = element_text(size = 10)) + xlab("PC") + ylab(TeX("Variance captured by PCs ($R^2_{k}$)"))
plLat
```
...
plInUKBB <- dfPlot %>% ggplot(aes(x = PC, y = R2, color = gwas)) + geom_point() + geom_line() + facet_wrap(~contrasts, nrow = 1) + geom_hline(aes(yintercept = Signal, color = gwas), linetype = "dashed") + theme_bw(base_size = 10) + scale_color_manual(values = c("purple1", "darkorange", "skyblue2", "seagreen"), name= TeX("GWAS"),  labels = c(expression(5 * epsilon), expression(10 * epsilon),expression(50 * epsilon),expression(200 * epsilon)))+ theme( legend.position = "bottom", strip.background=element_rect(colour="black",fill="white"), axis.text.x = element_text(size = 10)) + xlab("PC") + ylab(TeX("Variance captured by PCs ($R^2_{k}$)")) + ylim(0,1) +
  guides(color = guide_legend(nrow = 1))
