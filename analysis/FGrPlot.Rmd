...
```{r}
df$rho <- (df$H * ((1/df$signal) - 1)) - (1/df$L)
df$null <- 1/df$L + df$rho

# Bonferonni correction 
ci <- qnorm(1 -(0.025/nrow(df)))
df$lc <- df$H - (ci * sqrt(df$varH)) 
df$uc <- df$H + (ci * sqrt(df$varH)) 
```
