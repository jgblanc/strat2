...
dfBias <- dfBias %>% mutate(biasH = ((sigmaF2 *varTvec) / N)) %>% mutate(Hcorrected = H - biasH)  %>% mutate(lc = lc - biasH, uc = uc - biasH)


ggplot(data = dfBias, aes(x = H, y = Hcorrected)) + geom_point() + geom_abline(color = "red") + scale_x_log10() + scale_y_log10()
```
